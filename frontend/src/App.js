...
const ML_DESCRIPTION = [
  {
    title: 'Датасет',
    text: 'За основу взят публичный датасет соревнования Plant Pathology 2021 — FGVC8 (Kaggle): фотографии листьев яблони с мультилейбл-разметкой. Один лист может иметь сразу несколько диагнозов. Классы: healthy (здоровый), scab (парша), rust (ржавчина), frog_eye_leaf_spot (бурая пятнистость), powdery_mildew (мучнистая роса) и complex (комплекс болезней). Оригинальные изображения крупные (~4000 px, ~16 ГБ), поэтому для обучения они предварительно уменьшены до 512 px по длинной стороне с сохранением пропорций — иначе декодирование JPEG становится узким местом и GPU простаивает.'
  },
  {
    title: 'Модель',
    text: 'EfficientNet-B0 (свёрточная сеть, предобученная на ImageNet) с собственной классификационной головой: Dropout(0.3) + Linear на 6 выходов. Так как задача мультилейбл, на выходе применяется Sigmoid (независимая вероятность для каждого класса), а не Softmax. Функция потерь — BCEWithLogitsLoss с pos_weight для редких классов (один класс встречается всего ~70 раз), чтобы модель не игнорировала их.'
  },
  {
    title: 'Обучение',
  ...
  {
    title: 'Пороги по классам',
    text: 'Фиксированный порог 0.5 систематически невыгоден для редких классов: модель редко достигает высокой уверенности по классу, который видела ~70 раз, и с порогом 0.5 такой класс почти никогда не предсказывается. Поэтому пороги подобраны отдельно для каждого класса жадным перебором на валидации (greedy coordinate search в диапазоне 0.20–0.70), максимизируя mean F1. Эти пороги сохранены в чекпойнте и используются на инференсе.'
  },
  {
    title: 'Инференс',
    text: 'Изображение масштабируется до 256×256, нормализуется по статистикам ImageNet. Применяется TTA (test-time augmentation): каждый лист прогоняется через модель в трёх видах — оригинал, горизонтальный и вертикальный флип, — а вероятности усредняются. Лист ориентационно-инвариантен, поэтому усреднение убирает чувствительность модели к повороту и даёт небольшой бесплатный прирост. Классы, чья вероятность превышает свой порог, попадают в ответ; модель возвращает найденные болезни строкой.'
  },
  {
    title: 'Деплой',
