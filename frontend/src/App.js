...
const ML_DESCRIPTION = [
  ...
  {
    title: 'Деплой',
    text: 'Модель упакована в Docker-контейнер с FastAPI-сервисом инференса на PyTorch. Помимо диагноза, сервис для каждого выявленного заболевания дополнительно формирует тепловую карту Grad-CAM — она показывает, на какие участки листа смотрела модель, принимая решение по этому классу. Рядом развёрнут Node.js бэкенд на Express, который раздаёт галерею тестовых фото и проксирует запросы к инференсу. Фронтенд на React загружает фото (или снимок с камеры) и показывает найденные болезни, их вероятности и карту внимания модели.'
  }
];
